...
public class OSubscriber extends OutputSubscriber {
	...
	synchronized (this) {
		...
		OutputEvent currentOutputEvent = ((WebIOSession) handler
			.getUserSessions().get(
		...
		if (OutputEvent.PROP_SCREEN_RESOLUTION_MAX_X
		    ...
		    handler.updateScreenResolution(((Integer) newVal)
			    .intValue(), -1, -1, -1);
		    currentOutputEvent
		    ...
		    handler.updateScreenResolution(-1, ((Integer) newVal)
			    .intValue(), -1, -1);
		    currentOutputEvent
		    ...
		    handler.updateScreenResolution(-1, -1, ((Integer) newVal)
			    .intValue(), -1);
		    currentOutputEvent
		    ...
		    handler.updateScreenResolution(-1, -1, -1,
	...
	synchronized (handler.getWaitingInputs()) {
		...
		if (first.booleanValue()) {
		    log.debug("-------FIRST not null prvi: {}", first
			    .booleanValue());
		    handler.getReadyOutputs().put(user, event);// MAINMENU when
							       // asked
		    this.userDialogIDs.put(event.getDialogID(), user);
		    handler.getWaitingInputs().notifyAll();
		} else {
		    log.debug("-------FIRST not null drugi: {}", first
			    .booleanValue());

		    if (!event.getDialogType().equals(DialogType.sysMenu)) {
			log.debug("-------not sysMenu -> {}", event
				.getDialogType());
			handler.getReadyOutputs().put(user, event);// not
								   // MAINMENU
			this.userDialogIDs.put(event.getDialogID(), user);
			handler.getWaitingInputs().notifyAll();
		    }
		...
		handler.getReadyOutputs().put(user, event);// MAINMENU(or not)
							   // when not asked
							   // (recoveries)
		this.userDialogIDs.put(event.getDialogID(), user);
		handler.getWaitingInputs().notifyAll();
