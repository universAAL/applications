...
public class DojoHandler extends GatewayPort implements IWebHandler {
	...
	// SC2011:
	// change Modality.sms to Modality.gui
	// -> vraceno na sms da mogu paralelno raditi GUI i web handler
	...
	// SC2011: zakomentirano prema uputama iz maila
	...
	// }
	// // --Input processing--
	// log.info ("doPost Input processing: req.getHeader(Authorization)-> "
	...
	// SC2011: dodano prema uputama iz maila:
	String userURI = Constants.uAAL_MIDDLEWARE_LOCAL_ID_PREFIX + "nutritionist";

	log.info("Received HTTP request from user {} ", userURI);
	// Check if it is the first time
	
	//Ovo je Nikola radio sc2011 home submit
	boolean home = false;
	Enumeration nameS = req.getParameterNames();
	    String selectedsubmit = null;
	    for (; nameS.hasMoreElements();) {
		String name = (String) nameS.nextElement();
		if (name.startsWith("submit_home")) {
		    String submit = name.substring(7);
		    selectedsubmit = name.substring(7);
		    home = true;
		}
	    }
	    
	if (!userSessions.containsKey(userURI) || home) {
