 ...
 */
package org.universAAL.LivingRoom.unit_impl;

import java.util.ArrayList;
import java.util.Iterator;

import org.universAAL.middleware.container.utils.LogUtils;
import org.universAAL.LivingRoom.Activator;
...
public class MyLighting {
	...
	if (!myLampDB[lampID].isOn) {
	    ...
	    for (Iterator i = listeners.iterator(); i.hasNext();)
		((LampStateListener) i.next()).lampStateChanged(lampID,
			myLampDB[lampID].loc, 100);
	}
    }
    public void setBrightness(int lampID, int value) {
    	myLampDB[lampID].isOn = true;
		LogUtils.logInfo(Activator.mc, MyLighting.class, "setBrightness",
				new Object[] { "Lamp in ", myLampDB[lampID].loc,
						" set to "+value+"!" }, null);
		for (Iterator<LampStateListener> i = listeners.iterator(); i.hasNext();)
			i.next().lampStateChanged(lampID, myLampDB[lampID].loc, value);
	}
}
