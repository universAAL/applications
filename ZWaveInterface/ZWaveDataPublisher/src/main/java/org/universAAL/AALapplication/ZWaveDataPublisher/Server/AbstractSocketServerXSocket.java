 ...
 */
public abstract class AbstractSocketServerXSocket extends AbstractSocketServer implements IConnectHandler,
    IDisconnectHandler, IDataHandler, IIdleTimeoutHandler, IConnectionTimeoutHandler, ISocketServerConnectionListener {
     ...
     * 
     * @param activateConnectionLogging true to allow xSocket connection to log
     ...
     * 
     * @param protocolFactory protocol decoder factory to be used by the server
     * @param listenPort port to be used by the server to accept incoming
     *        connections
     */
     ...
     * @param idleTimeout milliseconds to receive some data, connection will be
     *        closed if no data is received. 0 to disable timeout.
     * @param connectionTimeout milliseconds to close the connection after being
     *        established. 0 to disable timeout.
     */
     ...
     * Socket connected.
     * 
     * @param connection new xSocket connection opened
     * @throws IOException error handling new connection
     ...
     * @return true if the event was handled
     * @throws IOException error handling connection timeout
     */
    public boolean onConnectionTimeout(final INonBlockingConnection connection) throws IOException {
        ...
        if (handler != null) {
            connectionInfo = " Connection Id: " + handler.getId() + " Pckts sent: "
                + handler.getConnectionSentPackets() + " Packts received: " + handler.getConnectionReceivedPackets();
        } else {
            connectionInfo = " ";
        }
     ...
     * 
     * @param connection the connection
     * @throws IOException error while processing incoming data.
     * @return true if the event was handled
     */
     ...
     * @throws IOException error handling disconnect event
     * @return true if the event was handled
     */
     ...
     * @throws IOException error handling idle timeout
     ...
     * @throws IOException error starting the server
     */
     ...
     * 
     * @param ip the IP to be used to bind the port
     */
