...
public class MealCategory extends ManagedIndividual {
    ...
    private static final String[] names = { "Breakfast", "Midmorning snack",
	    "Lunch", "Afternoon snack", "Dinner", "Afterdinner snack" };

    // These are all the possible instances of your enumeration: the individuals
    public static final MealCategory BREAKFAST = new MealCategory(
	    VALUE_BREAKFAST, 0);
    public static final MealCategory MIDMORNING_SNACK = new MealCategory(
	    VALUE_MIDMORNING_SNACK, 1);
    public static final MealCategory LUNCH = new MealCategory(VALUE_LUNCH, 2);
    public static final MealCategory AFTERNOON_SNACK = new MealCategory(
	    VALUE_AFTERNOON_SNACK, 3);
    public static final MealCategory DINNER = new MealCategory(VALUE_DINNER, 4);
    public static final MealCategory AFTERDINNER_SNACK = new MealCategory(
	    VALUE_AFTERDINNER_SNACK, 5);
	...
	default:
	    System.out.println("getMealCategoryByOrder return null para: "
		    + order);
	    return null;
	}
    }
	...
	return (instanceURI != null && instanceURI
		.startsWith(NutritionOntology.NAMESPACE)) ? valueOf(instanceURI
		.substring(NutritionOntology.NAMESPACE.length())) : null;
    }
