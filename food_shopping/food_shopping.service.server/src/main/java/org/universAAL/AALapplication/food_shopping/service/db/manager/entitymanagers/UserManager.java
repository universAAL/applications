...
public class UserManager extends EntityManager{
	...
	public Vector getShoppingListByName(Vector columns, Vector criteria) throws SQLException {
        ...
        if (columns.size() == 0){
            columns.add(new Column("food_shopping.shoppinglist","shoppinglist_id"));
            columns.add(new Column("food_shopping.shoppinglist","name"));
        }
        SelectStatement ss = new SelectStatement("food_shopping.shoppinglist",con);
        ...
        while (rs.next()){
            ResultRow rr = new ResultRow(rs);
            System.out.println("shopping list name="+rs.getString("SHOPPINGLIST.NAME"));
            result.add(rr);
        }
        rs.close();
