	...
	*/
	public void start(BundleContext bcontext) throws Exception {
		...
		try {
			...
			FileInputStream propIS = new FileInputStream(dataDir + "help_when_outdoor.properties");
			config.load(propIS);
		} catch (IOException e) {
		...
		HelpWhenOutdoorDataService dataService = new HelpWhenOutdoorXMLDataService(agenda);
		...
		wanderingdetector = new WanderingDetector(5,5);  // Checking user is out of safe area or is wandering
		
		// Service servlet for serving websites for consulting user data
		HelpWhenOutdoorServlet serviceServlet =
		new HelpWhenOutdoorServlet(
				dataService, 
				config);
