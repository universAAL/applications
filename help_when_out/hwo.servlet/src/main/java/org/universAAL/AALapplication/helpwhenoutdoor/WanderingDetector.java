...
public String isWandering (Point p, long timestamp){
	...
	if (timestamp-oldTimestamp<POSITION_SAMPLING_MSECS) //si la nueva posici�n no est� lo bastante separada temporalmente de la anterior, no hacemos nada.
		return "OK";
	...
	updateSegmentIntersections(); // Comprobamos si tenemos que a�adir intersecciones.
	...
	boolean resultW =lastSegmentIntersections>lastSegmentIntersectionThreshold
	 ...
	 //a�adido. Bloque para comprobar si el usuario est� fuera del �rea segura.
	...
	boolean resultC =  checkSafeArea(p);
	 ...
	 //a�adido. Bloque para comprobar si el usuario se ha ca�do.
...
public boolean checkSafeArea(Point p) {
	...
	//Es decir, trazando una linea paralela al eje X que pase por nuestra posici�n, vamos a comprobar si el numero de intersecciones con las rectas
	...
	//*** Paso 1: Ordenar las coordenadas de la SafeArea y averiguar los extremos.
	 ...
	 System.out.println("***Coordenadas X de la safearea  "+ VectorXsafearea[k]);
			//la m�s peque�a est� en el indice 1 y la m�xima est� en safearea.size(), sort aumenta el tama�o del vector en 1 xD
		}
	
	//Ahora, si x es menor que la menor coordenada X de los puntos de la safearea o mayor que la m�xima coordenada X, es seguro que estamos fuera. Si est� entre ellas, hay que comprobarlo.
	
	 //*** Paso 2: Trazar rectas entre todos los puntos contiguos de la safearea y contar cu�ntas veces intersectan con la paralela al eje x desde xpos a un extremo.
...
private boolean stopped(Vector last5points) {
	...
	RoutePoint pointB= (RoutePoint)last5points.get(4); //primer y �ltimo punto de una serie de 5. Por tanto, desplazamiento en 5 min.
...
private double roundtodec(double decimal,int numberofdecs) { //funci�n que redondea a X decimales. Cruzar una calle hace que cambien X decimales de la posici�n. En 5 minutos deber�a
	//haber recorrido esa distancia. Por tanto, redondeando las coordenadas a X decimales, si son id�nticas, significa que la posici�n ha variado muy poco
	//y el usuario puede haberse detenido o ca�do. Nota: X= 3 � 4, comprobar con un GPS real.
