...
public class OutputPublisher extends
	...
	private void addAdaptationParams(OutputEvent event, String queryStr) {
		...
		//SC2011: komentari
		/*
		 * Ova metoda sluzi da eventu koji sadrzi formu doda popratne parametre vezane uz trenutnog korisnika
		 * Izmedu ostalog dodaje se parametar modality prema kojem se kasnije bira handler
		 * Za gui handler se koristi Modality.gui, a za web Modality.sms pretpostavljam zbog netostatka pametnijeg izbora
		 * 
		 * Informacije o korisnicima se citaju iz baze, a spajanje na nju je odredeno u razredu Activator.
		 * Ako korisnika koji trazi formu nema u bazi unose se testni podaci pomocu metode Activator.loadTestData()
		 * Njoj se prije predavao samo uri korisnika, a sada se daje i modality.
		 * 
		 * dm.mobile u ovoj metodi ima samo liniju:
		 * 		event.setOutputModality(Modality.gui);
		 * Tako da se tamo sve salje na GUI handler
		 * 
		 */
		//SC2011: hardkodirano modality za korisnike
//		if (event.getAddressedUser().getURI().endsWith("nutritionist")) event.setOutputModality(Modality.sms);
//		if (event.getAddressedUser().getURI().endsWith("saied")) event.setOutputModality(Modality.gui);
		
		//SC2011:
//		System.err.println("preskoceno addAdaptationParams");
//		if (0 == 0) return;
		
		//SC2011:
		//System.err.println("addAdaptationParams => " + queryStr);
		if (queryStr == null)
		...
		try {
			...
			if (con.containsModel(Activator.JENA_MODEL_NAME)) {
				...
				if (root == null) {
					...
					System.err.println("prije ucitavanja testData");
					Activator.loadTestData(
							Constants.uAAL_MIDDLEWARE_LOCAL_ID_PREFIX
							+ "saied", //$NON-NLS-1$
							"Saied", Modality.gui); //$NON-NLS-1$
					 
					//SC2011: dodano za nutritionist
					Activator.loadTestData(
							Constants.uAAL_MIDDLEWARE_LOCAL_ID_PREFIX+ "nutritionist", //$NON-NLS-1$
							"Nutritionist", Modality.sms); //$NON-NLS-1$
					
					Activator.loadTestData(
							"urn:org.aal-persona.profiling:12345:ella", //$NON-NLS-1$
							"Ella", Modality.gui); //$NON-NLS-1$
					Activator.loadTestData(
							"urn:org.aal-persona.profiling:12345:john", //$NON-NLS-1$
							"John", Modality.gui); //$NON-NLS-1$
				...
				if (pr instanceof ElderlyUser) {
					...
					if (up instanceof ElderlyProfile) {
						...
						if (ppp != null) {
							...
							System.err.println("MODALITY: "+ppp.getXactionModality().toString());
