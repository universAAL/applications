...
public class BiomedicalSensorsServiceProfiles extends BiomedicalSensorService {
    ...
    public static final String MY_URI = BMS_SERVER_NAMESPACE
	    + "BMSService";
    ...
    static final String SERVICE_SENSOR_BT_SERVICE_URL = BMS_SERVER_NAMESPACE
	    + "getBTserviceURL";
    static final String SERVICE_GET_MEASUREMENTS = BMS_SERVER_NAMESPACE + "getMeasurements";
    static final String SERVICE_START_WAIT_FOR_ALERTS = BMS_SERVER_NAMESPACE + "startWaitForAlerts";
    static final String SERVICE_STOP_WAIT_FOR_ALERTS = BMS_SERVER_NAMESPACE + "stopWaitForAlerts";
	...
	OntologyManagement.getInstance().register(
		new SimpleOntology(MY_URI, BiomedicalSensorService.MY_URI,
			new ResourceFactoryImpl() {
				...
				return new BiomedicalSensorsServiceProfiles(instanceURI);
			    }
			}));
	...
	String[] ppmonitorsAlerts = new String[] { BiomedicalSensorService.PROP_CONTROLS,
			AlertService.PROP_MONITORS };
	String[] ppSensorType = new String[] { BiomedicalSensorService.PROP_CONTROLS,
			CompositeBiomedicalSensor.PROP_SENSOR_TYPE };
	String[] ppBTURL = new String[] { BiomedicalSensorService.PROP_CONTROLS,
			CompositeBiomedicalSensor.PROP_DISCOVERED_BT_SERVICE };
	String[] ppMeasurements = new String[] { BiomedicalSensorService.PROP_CONTROLS,
	...
	addRestriction((MergedRestriction) BiomedicalSensorService
		.getClassRestrictionsOnProperty(BiomedicalSensorService.MY_URI,
				BiomedicalSensorService.PROP_CONTROLS).copy(), ppControls,
			serverBiomedicalSensorsRestrictions);

	/*addRestriction(MergedRestriction.getFixedValueRestriction(
			CompositeBiomedicalSensor.PROP_CONNECTION_TYPE, Zephyr.btSensor),
			new String[] { BiomedicalSensorService.PROP_CONTROLS,
			CompositeBiomedicalSensor.PROP_CONNECTION_TYPE }, serverBiomedicalSensorsRestrictions);*/
	/*	addRestriction(MergedRestriction.getFixedValueRestriction(
			CompositeBiomedicalSensor.PROP_SENSOR_TYPE, BPmonitor.OMRON),ppSensorType, 
			serverBiomedicalSensorsRestrictions);*/
	...
	// Filtering methog in order to get info for the sensor with the specific URI
	getSensorInfo.addFilteringInput(INPUT_SENSOR_URI, CompositeBiomedicalSensor.MY_URI, 1, 1,
		ppControls);
	getSensorInfo.addOutput(OUTPUT_SENSOR_INFO, TypeMapper
		.getDatatypeURI(String.class), 1, 1, ppSensorType);
	getSensorInfo.addOutput(OUTPUT_DISCOVERED_SERVICE, TypeMapper
			.getDatatypeURI(String.class), 1, 1, ppBTURL);
	// Adding service profile to be registered with the service bus
	...
	getBTServiceURL.addFilteringInput(INPUT_SENSOR_URI, CompositeBiomedicalSensor.MY_URI, 1, 1,
			ppControls);
	getBTServiceURL.myProfile.addChangeEffect(ppBTURL, new Boolean(true));		
	getBTServiceURL.addOutput(OUTPUT_DISCOVERED_SERVICE, TypeMapper
			.getDatatypeURI(String.class), 1, 1, ppBTURL);		
	// Adding service profile to be registered with the service bus
	...
	getMeasurements.addFilteringInput(INPUT_SENSOR_URI, CompositeBiomedicalSensor.MY_URI, 1, 1,
			ppControls);
	getMeasurements.myProfile.addChangeEffect(ppMeasurements, new Boolean(true));		
	getMeasurements.addOutput(OUTPUT_SENSOR_MEASUREMENTS, TypeMapper
				.getDatatypeURI(String.class), 0, 0, ppMeasurements);
	// Adding service profile to be registered with the service bus	
	...
	startWaitForAlerts.addFilteringInput(INPUT_SENSOR_URI, CompositeBiomedicalSensor.MY_URI, 1, 1,
			ppmonitorsAlerts);				
	startWaitForAlerts.myProfile.addChangeEffect(ppmonitorsAlerts, new Boolean(true));
	// Adding service profile to be registered with the service bus
	...
	stopWaitForAlerts.addFilteringInput(INPUT_SENSOR_URI, CompositeBiomedicalSensor.MY_URI, 1, 1,
						ppmonitorsAlerts);					
	stopWaitForAlerts.myProfile.addChangeEffect(ppmonitorsAlerts, new Boolean(false));
