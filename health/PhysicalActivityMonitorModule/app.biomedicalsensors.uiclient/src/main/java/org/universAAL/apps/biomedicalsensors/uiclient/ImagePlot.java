...
int PAD=40;
	...
	public  ImagePlot(String fpOutfile,double[] plotData,String dataLabel, String axisX,String axisY,int w,int h){
		double[] normData;
	     ...
	     g2.clearRect(0, 0, w, h);
	        ...
	        g2.draw(new Line2D.Double(PAD, h-PAD-scale*Math.abs(getMin(plotData)), w-PAD, h-PAD-scale*Math.abs(getMin(plotData))));
	        }else{
	        ...
	        if (getMin(plotData)<0){
            g2.drawString("0", (int)(PAD-font.getStringBounds("0", frc).getWidth()),(int) ((h-PAD-scale*Math.abs(getMin(plotData))+(lm.getAscent()/3))));
	        }else{
	        	g2.drawString("0", (int)(PAD-font.getStringBounds("0", frc).getWidth()),(int) ((h-PAD+(lm.getAscent()/3))));
	        }
	        }
	        ...
	        for(int i = 0; i < normData.length-1; i++) {
	        	...
	        	g2.setPaint(Color.DARK_GRAY);
	            g2.drawString(String.valueOf(plotData[0]),(int) (w/2+PAD),(int) (h - PAD - scale*plotData[0]));
	            g2.setPaint(Color.green.darker());
	        }
	        // Mark data points.
