...
public class OntoFactoryTester {
	...
	try {
	    ...
	    String token;
	    token = ws.getToken("David_Shopland",
		    "5a019bc5dad45f1652c141bc9004598b", 2);
		...
		for (Ingredient ingredient : ings) {
			...
			Utils.println("Ingredient: " + ingredient.getQuantity()
				+ " " + ingredient.getFood().getName()
				+ ingredient.getMeasureUnit().getType());
		    if (ingredient != null
			    && ingredient.getMeasureUnit() != null)
			Utils.println("Ingredient: "
				+ ingredient.getMeasureUnit().getType());
		}
	    } else {
		Utils.println("Not found with ID = " + ID);
	    ...
	    ShoppingList shop = ws.getCustomShoppingListDays(token, 1,
		    Utils.Dates.getMyCalendar().DAY_OF_WEEK);
	    if (shop != null) {
		Utils.println("Found shopping list: " + shop.getItems().length
			+ " items");
		int i = 0;
		for (FoodItem item : shop.getItems()) {
		    Utils.println(" item: " + i + " " + item.getFoodID() + " "
			    + item.getName());
		    i++;
		}
	    } else {
