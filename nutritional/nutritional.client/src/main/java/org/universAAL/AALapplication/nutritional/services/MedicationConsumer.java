...
public final class MedicationConsumer {
    ...
    // private static final String OUTPUT_PRECAUTION =
    // PRECAUTION_CONSUMER_NAMESPACE + "precaution";

    private static final String OUTPUT_PRECAUTION_SIDEEFFECT = PRECAUTION_CONSUMER_NAMESPACE
	    + "sideeffect";
    private static final String OUTPUT_PRECAUTION_INCOMPLIANCE = PRECAUTION_CONSUMER_NAMESPACE
	    + "incompliance";
	...
	ServiceRequest serviceRequest = new ServiceRequest(new Precaution(),
		user);

	// serviceRequest.addRequiredOutput(OUTPUT_PRECAUTION, new
	// String[]{Precaution.SIDEEFFECT, Precaution.INCOMPLIANCE});

	serviceRequest.addRequiredOutput(OUTPUT_PRECAUTION_SIDEEFFECT,
		new String[] { Precaution.PROP_SIDEEFFECT });
	serviceRequest.addRequiredOutput(OUTPUT_PRECAUTION_INCOMPLIANCE,
		new String[] { Precaution.PROP_INCOMPLIANCE });
	...
	List list = serviceResponse.getOutput(OUTPUT_PRECAUTION_SIDEEFFECT,
		true);
	if (list != null && !list.isEmpty()) {
	    if (list.size() == 1) {
		Utils.println("Medication SIDE1");
		p.setProperty(Precaution.PROP_SIDEEFFECT,
			((Precaution) list.get(0)).getSideEffect());
	    } else {
		Utils.println("Medication SIDEN");
		p.setProperty(Precaution.PROP_SIDEEFFECT, list);
	    }
	} else {
	    Utils.println("Medication SIDEnull");
	    return null;
	}
	list = serviceResponse.getOutput(OUTPUT_PRECAUTION_INCOMPLIANCE, true);
	if (list != null && !list.isEmpty()) {
	    if (list.size() == 1) {
		Utils.println("Medication INCOM1");
		p.setProperty(Precaution.PROP_INCOMPLIANCE,
			((Precaution) list.get(0)).getIncompliance());
	    } else {
		Utils.println("Medication INCOMN");
		p.setProperty(Precaution.PROP_INCOMPLIANCE, list);
	    }
	} else {
	    Utils.println("Medication INCOMnull");
	    return null;
	}
	return p;
    }
