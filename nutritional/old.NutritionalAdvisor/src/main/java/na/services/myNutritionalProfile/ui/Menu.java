...
public class Menu extends SecNavigationBar {
	...
	protected NutriProfileSubServiceLauncher launcher;
	
	public Menu() {
		GridBagLayout gridBagLayout = new GridBagLayout();
		gridBagLayout.columnWidths = new int[]{100, 0};
		gridBagLayout.rowHeights = new int[]{5, 50, 50, 0};
		...
		setLayout(gridBagLayout);
		{
			...
			button.addActionListener(//new ActionButton(new ActionListener() {          
					new ActionListener() {
			    public void actionPerformed(ActionEvent e) {
			    	butMyProfile(e);
			    }
			}); 
			ButtonLab.getInstance().addObject(button, ButtonLab.profile_eating);
			button.setText(Messages.Profile_My_Eating_Preferences);
//			button.setText("eating prefe");
		...
		}
		{
			...
			button_1.addActionListener(new ActionListener() {          
			    public void actionPerformed(ActionEvent e) {
			    	butPendingQuestionnaires(e);
			    }
			});
			button_1.setText(Messages.Profile_PendingQuestionaire);
//			button_1.setText("Pending");
			ButtonLab.getInstance().addObject(button_1, ButtonLab.profile_pending_questionnaires);
	...
	private void butPendingQuestionnaires(ActionEvent evt) {
		this.select((AdaptiveButton)evt.getSource());
		launcher.showPendingQuestionnaires();
	}

	private void butMyProfile(ActionEvent evt) {
		this.select((AdaptiveButton)evt.getSource());
		launcher.showUserNutritionalProfile();
	}
