...
import na.utils.ws.EnvironmentWSConnector;
import na.utils.ws.NutritionalAdvisorWSConnector;
import na.utils.ws.SocialCommunityWSConnector;
...
//	private Map<String, IdealOperationDescriptor> na_ideal_ops = new HashMap<String, IdealOperationDescriptor>();
//	private Map<String, IdealOperationDescriptor> environment_ideal_ops = new HashMap<String, IdealOperationDescriptor>();
	...
	private AmiConnector(boolean forceUseAMI) {
		...
		log.info("AMI: >>>>>>>>>>>>> AMI took: "+ seconds + " seconds to start");
	}
	
	
	
	/**
	 * Invokes an operation. Depending on the attribute on user.properties avoid_ami it will avoid using the ami or not
	 * 
	 * @param operationName the operation name
	 * @param input3 the input
	 * @param forceWebServiceCall 
	 * @return the object
	 * @throws ServiceUnavailableException 
	 * @throws RemoteException 
	 */
	public Object invokeOperation(String domain, String operationName, Object[] input3, boolean forceWebServiceCall) throws OASIS_ServiceUnavailable { 
	...
	/**
	 * Calls the Nutritional Advisor Web Service directly, without using OASIS AMI framework.
	 * 
	 * @param operationName the operation name
	 * @param input3 the input
	 * @return the object
	 * @throws RemoteException 
	 */
	private Object directWSInvokeOperation(String domain, String operationName, Object[] input3){
		if (domain.compareTo(ServiceInterface.DOMAIN_Nutrition)==0) {
			if (operationName.compareTo(ServiceInterface.OP_GetToken)==0) {
				...
				Object output;
				try {
					output = nutritionalWS.invokeOperation(operationName, 1, input3);
					if (output != null) {
						return output;
					}
					else 
...
//						e.printStackTrace();
		...
		} else if (domain.compareTo(ServiceInterface.DOMAIN_SocialCommunity)==0) {
			SocialCommunityWSConnector socialWS = new SocialCommunityWSConnector();
			Object output;
			...
			if (output != null) {
				...
				log.info("AMI: No output found.");
		} else if (domain.compareTo(ServiceInterface.DOMAIN_EnvironmentalControl)==0) {
...
//	public void testTemperature() throws RequestTimeExceedException, RemoteCallException {
...
//			long start = System.nanoTime();
//			List<AlignedOperationDescriptor> alignedOps = this.frontEnd.requestAlignedOperations(idealOperation);			
...
////					continue;
////				}
//				WSOperationInput wsInput = this.frontEnd.requestAlignedOperationInput(alignedOp);
...
//				WSOperationOutput output = this.frontEnd.requestAlignedOperationResult(alignedOp);
