...
public class Login {
	...
	/**
	 * Login user into the system using username, password and preferredLanguage.
	 * Username and password retrieved from user.properties file
	 * preferredLanguage from Profile.
	 */
	public boolean logMeIn() {
		// read credentials from file
		String username = ProfileConnector.getInstance().getNutritionalUsername();
		String password = ProfileConnector.getInstance().getNutritionalPassword();
		...
		try {
			// ask login
			log.info("asking login with username: "+username + " lang: "+preferredLanguage);
			String[] input = {username, password, ""+preferredLanguage};
			AmiConnector ami = AmiConnector.getAMI();
			if (ami != null) {
				String token = (String)ami.invokeOperation(ServiceInterface.DOMAIN_Nutrition, ServiceInterface.OP_GetToken, input, false);
...
//				log.info(" token recibido: "+TSFConnector.getInstance().getToken());
			...
			e.printStackTrace();
			log.fatal("could not authenticate, service unavailable: "+e.getMessage());
		}
		return false;
	}
