...
/**
 * The UI class that builds the Shopping Form and handles its associated response
 * submits.
 * 
 * @author alfiva
 * 
...
public class UIShopping {
	...
	Form f = Form.newDialog(Messages.getString("UIShopping.0"), new Resource()); //$NON-NLS-1$
	f.setProperty("http://ontology.itaca.es/ClassicGUI.owl#layout", "vertical,left");
	InputField in = new InputField(f.getIOControls(), new Label(
		Messages.getString("UIShopping.1"), null), PROP_PATH_REF1, null, //$NON-NLS-1$
		null);
//	in.setAlertString("Expected: a number between 0 and 100!");

	TextArea ta = new TextArea(f.getIOControls(), new Label(
		Messages.getString("UIShopping.2"), null), PROP_PATH_REF2, null, null); //$NON-NLS-1$
//	ta.setAlertString("Aqui falta algo");
//	ta.setHelpString("Ayuda para el text");
	new SimpleOutput(f.getIOControls(), null, null, Messages.getString("UIShopping.3")); //$NON-NLS-1$

	new Submit(f.getSubmits(), new Label(Messages.getString("UIShopping.4"), null), SUBMIT_GOBACK); //$NON-NLS-1$
