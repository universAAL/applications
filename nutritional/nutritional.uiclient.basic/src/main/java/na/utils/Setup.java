...
//	protected static final String DEFAULT_PATH_Nutritional_DESCRIPTOR = Constants.getSpaceConfRoot() + "/NutritionalAdvisor";
	private static String absolutePath = new BundleConfigHome("nutritional.uiclient").getAbsolutePath();

	protected static final String DEFAULT_PATH_Nutritional_DESCRIPTOR = absolutePath + "/NutritionalAdvisor";
...
//	private static final String AMI_UserID = "itaca_01"; 
	...
	private static final double DEFAULT_MAX_TEMPERATURE_VALUE = 25; // same as InitialSetup.max_temp_value!!
...
//		String setupFileName = file.getAbsolutePath() + "\\"+ ServiceInterface.DIR_NUTRITIONAL_ROOT +"\\"+ServiceInterface.SETUP_FILENAME;
...
////			return dir1.getCanonicalPath()+"\\..\\"+ServiceInterface.SETUP_FILENAME;
////			setupFileName = dir1.getCanonicalPath()+"\\NutritionalAdvisor\\"+ServiceInterface.SETUP_FILENAME;
////			setupFileName = dir1.getCanonicalPath()+ "\\" + ServiceInterface.DIR_NUTRITIONAL_ROOT +"\\"+ServiceInterface.SETUP_FILENAME;
//			setupFileName = ServiceInterface.DIR_NUTRITIONAL_ROOT +"\\"+ServiceInterface.SETUP_FILENAME;
//			return setupFileName;
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
//		return null;
	...
	static public String getServerIP() {
		...
		try {
		    ...
		    String url = properties.getProperty(InitialSetup.WEB_SERVER_IP_ADDRESS_KEY);
		    if (url==null) {
	...
	static public String getWebServiceAddress() {
		if (Setup.WEB_SERVICE_ADDRESS!=null)
			...
			try {
				String setup = getSetupFileName();
			    properties.load(new FileInputStream(setup));
			    String server_ip = properties.getProperty(InitialSetup.WEB_SERVER_IP_ADDRESS_KEY);
			    String server_port = properties.getProperty(InitialSetup.WEB_SERVER_PORT_KEY);
			    String server_webService_address = properties.getProperty(InitialSetup.WEB_SERVER_SERVICE_ADDRESS_KEY);
			    if ((server_ip == null) || (server_port == null) || (server_webService_address == null)) {
			    	log.info("Couldn't get web server properties. Found: ServerIP: " + server_ip + " ServerPort: "+server_port + " webServiceName: "+server_webService_address);
			    } else {
			    	String result = "http://"+server_ip+":"+server_port+"/"+server_webService_address;
	...
	public static boolean isMaximizedActive() {
		...
		try {
			String setup = getSetupFileName();
		    properties.load(new FileInputStream(setup));
		    String url = properties.getProperty(InitialSetup.MAXIMIZE_WINDOW_KEY);
		    if (url==null) {
		    	log.info("Couldn't find setup.properties file at: "+setup);
		    	return false;
		    } else {
	...
	public static boolean isScreenSaverActive() {
		...
		try {
			String setup = getSetupFileName();
		    properties.load(new FileInputStream(setup));
		    String url = properties.getProperty(InitialSetup.SCREENSAVER_ENABLED_KEY);
		    if (url==null) {
		    	log.info("Couldn't find setup.properties file at: "+setup);
		    	return false;
		    } else {
...
//			log.info("setup is in: "+setup);
		    properties.load(new FileInputStream(setup));
		    String url = properties.getProperty(InitialSetup.CHECK_ADVISES_DELAY_MINUTES_KEY);
		    if (url==null) {
		    	log.info("Couldn't find check_advises_delay_minutes in setup.properties file: "+setup);
		    	return DEFAULT_DOWNLOAD_ADVICES_EVERY_MINUTES;
		    } else {
		    	log.info("--------------------> Advises delay to ask server: "+url);
...
//			log.info("setup is in: "+setup);
		    properties.load(new FileInputStream(setup));
		    String url = properties.getProperty(InitialSetup.MAX_TEMPERATURE_KEY);
		    if (url==null) {
		    	log.info("Couldn't find max_temp in setup.properties file: "+setup);
	...
	public static int getScreensaverDelay() {
		...
		try {
			String setup = getSetupFileName();
		    properties.load(new FileInputStream(setup));
		    String url = properties.getProperty(InitialSetup.SCREENSAVER_DELAY_SECONDS_KEY);
		    if (url==null) {
		    	log.info("Couldn't find setup.properties file at: "+setup);
...
//		return "BU";
		...
		try {
			String setup = getSetupFileName();
		    properties.load(new FileInputStream(setup));
		    String lang = properties.getProperty(InitialSetup.LANGUAGE_INTERFACE_KEY);
		    if (lang!=null)
		    	return lang;
		} catch (IOException e) {
			log.error("Error, couldn't read file: "+e.toString());
	...
	public static boolean getShowEmptyCache() {
		...
		try {
			String setup = getSetupFileName();
		    properties.load(new FileInputStream(setup));
		    String value = properties.getProperty(InitialSetup.SHOW_EMTPY_CACHE_KEY);
		    if (value==null) {
		    	return false;
		    } else {
		    	if (value.compareTo("yes")==0)
...
//	}
	...
	public static String getPrinterConfFile() {
		...
		log.info("Default printer conf file at: "+InitialSetup.PrinterConf_VALUE);
	...
	static public boolean download_profile_on_start() {
		...
		try {
			String setup = getSetupFileName();
		    properties.load(new FileInputStream(setup));
		    String value = properties.getProperty(InitialSetup.DOWNLOAD_PROFILE_ON_START);
		    if (value==null) {
		    	return false;
		    } else {
		    	if (value.compareTo("yes")==0)
	...
	static public boolean use_WS_toConnectToSocialCommunities() {
		...
		try {
			String setup = getSetupFileName();
		    properties.load(new FileInputStream(setup));
		    String value = properties.getProperty(InitialSetup.SOCIAL_COMMUNITI_VIA_WEB_KEY);
		    if (value==null) {
	...
	static public boolean use_Scheduler() {
		...
		try {
			String setup = getSetupFileName();
		    properties.load(new FileInputStream(setup));
		    String value = properties.getProperty(InitialSetup.USE_SCHEDULER_KEY);
		    if (value==null) {
	...
	public static void storeToken(String token2) {
		...
		try {
		    ...
		    properties.store(new FileOutputStream(Setup.getSetupFileName()), null);
