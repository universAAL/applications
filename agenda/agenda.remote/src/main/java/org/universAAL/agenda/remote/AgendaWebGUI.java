...
public class AgendaWebGUI {
	...
	if (calendars.isEmpty()) {
	    ...
	    // Group remrepeatgroup=new Group(invisiblegroup2,new
	    // org.uAAL.middleware.dialog.Label("Repeat",(String)null),null,null,(Resource)null);
	    // Select1 remrepeatselect = new Select1(remrepeatgroup, new
	    // org.uAAL.middleware.dialog.Label("Times    ",(String)null),
	    // new PropertyPath(null, false, new String[]{REF_REM_REP}),
	    // null,null);
	    // for(int i=1;i<10;i++){
	    // remrepeatselect.addChoiceItem(new
	    // ChoiceItem(Integer.toString(i),(String)null,new Integer(i)));
	    // }
	    // Select1 remintervalselect = new Select1(remrepeatgroup, new
	    // org.uAAL.middleware.dialog.Label("Interval ",(String)null),
	    // new PropertyPath(null, false, new String[]{REF_REM_INT}),
	    // null,null);
	    // for(int i=1;i<60;i++){
	    // remintervalselect.addChoiceItem(new
	    // ChoiceItem(Integer.toString(i),(String)null,new Integer(i)));
	    // }
    ...
    // SC2011 Events form
    public Form getEventsForm() {
	    System.err.print("USAO SAM U FORMU ZA EVENTE");
		Form f = Form.newDialog(SCREEN_TITLE, (String) null);
		Group controls = f.getIOControls();
		Group submits = f.getSubmits();
...
//		    .getString("AgendaWebGUI.2"));
	//	
	//}
		System.err.println("PRije citanja eventova");
		
	FilterParams params= new FilterParams();
		List<Event> events = Activator.guicaller
		.requestEventListService(new User(
			Constants.uAAL_MIDDLEWARE_LOCAL_ID_PREFIX + "saied"));

		System.err.println("Poslje citanja eventova");
		
	Collections.sort(events, new MyEventComparator());
	System.err.println("Procitani eventovi u agendaWebGUI_Event---:"+events);
	System.err.println("Broj eventeova u  agendaWebGUI_Event---:"+events.size());


	   
	    new Submit(submits, new org.universAAL.middleware.io.rdf.Label(
		    Messages.getString("AgendaWebGUI.26"), (String) null),
		    "Event_editor");

	    //Pocetak stvaranja forme
		
	    Group eventsGroup = new Group(controls,
		    new org.universAAL.middleware.io.rdf.Label("Events", (String) null),
		    null, null, (Resource) null);
		...
		Group eventG = new Group(invisiblegroup,
			    new org.universAAL.middleware.io.rdf.Label("Event: " + event.getEventDetails().getTimeInterval().getStartTime().getDay()
				                                                 + "/" + event.getEventDetails().getTimeInterval().getStartTime().getMonth()
				                                                 + "/" + event.getEventDetails().getTimeInterval().getStartTime().getYear(), 
				                                                 (String) null),
				                                                 null, null, (Resource) null);
		Group invisiblegroup1 = new Group(eventG, null, null, null,
			    (Resource) null);
		new SimpleOutput(invisiblegroup1, null, null, "Category: " + event.getEventDetails().getCategory());
		new SimpleOutput(invisiblegroup1, null, null, "Description: " + event.getEventDetails().getDescription());
		new SimpleOutput(invisiblegroup1, null, null, "Place Name: " + event.getEventDetails().getPlaceName());
		new SimpleOutput(invisiblegroup1, null, null, "Spoken language: " + event.getEventDetails().getSpokenLanguage());
		new SimpleOutput(invisiblegroup1, null, null, "Begin time: " + hour(event.getEventDetails().getTimeInterval().getStartTime().getHour())
								    + ":" + minute(event.getEventDetails().getTimeInterval().getStartTime().getMinute()));
		//new SimpleOutput(eventG, null, null, "End time: " + event.getEventDetails().getTimeInterval().getEndTime().getHour()
		//	    					  + ":" + event.getEventDetails().getTimeInterval().getEndTime().getMinute());
	    }
	...
	else return((new Integer(hour)).toString());
    }
   public String minute (int minute){
	...
	else return((new Integer(minute)).toString());
    } 
