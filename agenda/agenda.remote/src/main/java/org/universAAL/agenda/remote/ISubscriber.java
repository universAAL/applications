...
public class ISubscriber extends InputSubscriber {
    ...
    protected ISubscriber(ModuleContext mcontext) {
	super(mcontext);
    }
    ...
    /**
     * @see org.universAAL.middleware.input.InputSubscriber#handleInputEvent(org.universAAL
	...
	try {
		...
		Activator.guioutput.showEventsScreen(user, cal);
		// uzima evente iz odredenog kalendara
		...
		if (Activator.guicaller.deleteCalendarEventService(cal, ev
		    ...
		    Activator.guioutput.showMessageScreen(user, Messages
			    .getString("ISubscriber.7"));
		} else {
		    log.debug("Event could not be removed");
		    Activator.guioutput.showMessageScreen(user, Messages
			    .getString("ISubscriber.8"));
		}
		...
		Activator.guioutput.showGoogleScreen(user, cal);
