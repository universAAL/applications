...
public class JEventList implements PersonaWindow {
	...
	public DateInstance getSelectedDate() {
		...
		if (this.selectedDate == null) {
			...
			return new DateInstance(c.get(Calendar.YEAR), c.get(Calendar.MONTH), c.get(Calendar.DAY_OF_MONTH));
		} else
			return this.selectedDate;
	}
	
	private JPanel createTitleScreen() {
		ImageIcon persona_logo = new ImageIcon(getClass().getResource(Activator.ICON_PATH_PREFIX + "/month_header.jpg")); //$NON-NLS-1$
		...
		headPanel.setBackground(Color.white);
	  	...
	  	//SC2011: breadcrumbs
	  	JLabel l = new JLabel("<html><font face=\"MyriadPro\" size=\"5\" >Home -> Event list</font>");
	...
	public JPanel createNavScreen() {
		...
		ImageIcon add_icon = new ImageIcon(getClass().getResource(Activator.ICON_PATH_PREFIX + "/add.jpg")); //$NON-NLS-1$
		ImageIcon back_icon = new ImageIcon(getClass().getResource(Activator.ICON_PATH_PREFIX + "/back.jpg")); //$NON-NLS-1$
		ImageIcon next_icon = new ImageIcon(getClass().getResource(Activator.ICON_PATH_PREFIX + "/next.jpg")); //$NON-NLS-1$
		ImageIcon main_icon = new ImageIcon(getClass().getResource(Activator.ICON_PATH_PREFIX + "/main.jpg")); //$NON-NLS-1$
		...
		back.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				activeCalendarScreen.populateCalendar(activeCalendarScreen.calendar);
		...
		//nav.add(new JLabel()); //<<Create a void combined with nav.setLayout(new GridLayout(6, 1));
	...
	public void updateMainScreen(List<Event> events) {
		...
		for (Event event : events) {
			...
			if (counter % 3 == 0) {
				this.mainScreen.add(EVENT_TAB_NAME + (counter/3) , middle); //eventTab1, 2,...
				middle = new JPanel();
				middle.setLayout(new GridLayout(4,1));
				middle.setBackground(Color.white);
				incomplete = false;				
	...
	public void updateTitleScreen(int eventNo, int year, int month, int day) {
		...
		if (eventNo < 1) {
			eventDescription = Messages.getString("JEventList.NoScheduledAppointment"); //$NON-NLS-1$
		} else if (eventNo == 1){
		...
		headerMessage.setText("<html><font face=\"MyriadPro\" size=\"7\" color=\"white\">" + day //$NON-NLS-1$
	...
	public void updateTitleScreen(int eventNo) {
		...
		headerMessage.setText("<html><font face=\"MyriadPro\" size=\"7\" color=\"white\">"   //$NON-NLS-1$
	...
	public void overrideTitle(String headerText, String explanatoryText) {
		headerMessage.setText("<html><font face=\"MyriadPro\" size=\"7\" color=\"white\">" + headerText //$NON-NLS-1$
				+ "</font><br/>"  //$NON-NLS-1$
	...
	class AppointmentEntry extends JPanel {
			...
			if (event.getEventDetails() != null){
				...
				if (ti != null) {
					...
					if (start != null) {
						...
						if (startM < 10)
							m = "0"; //$NON-NLS-1$
						this.hour = h + start.getHour() + ":" + m + start.getMinute(); //$NON-NLS-1$
		...
		public void initComponents() {
			...
			String labelText = 	"<Html>" + type + "<br/>"  //$NON-NLS-1$ //$NON-NLS-2$
								+ hour  + "&nbsp;&nbsp;<font size=\"5\" color=\"blue\">" + date + "</font><br/>"  //$NON-NLS-1$ //$NON-NLS-2$
			...
			moreInfo = new JLabel(Messages.getString("JEventList.ClickForMoreInfo")); //$NON-NLS-1$
			...
			addMouseListener(new MouseAdapter() {
				...
				public void mouseExited(MouseEvent e) {
					setBorder(BorderFactory.createEtchedBorder(Color.white, Color.white));
				}
