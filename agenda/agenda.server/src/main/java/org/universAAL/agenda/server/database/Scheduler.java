...
public class Scheduler {
	...
	public static final int DEFAULT_REPEAT_INTERVAL = 30*1000; //half-min (ten minutes) repeat interval
	private Timer dbPeriodicalCheck;
	private TimerTask checkForReminders, checkForStartingEvents, checkForEndingEvents;
	private Timer publishContextEvents;
	private AgendaDBInterface dbServer;
	private AgendaStateListener agendaListener;
	private Map forthcomingReminders, forthcomingStartEvents, forthcomingEndEvents;

	public Scheduler(AgendaDBInterface dbServer, AgendaStateListener agendaListener) {
	...
	private void initTimers(AgendaDBInterface dbServer) {
		...
		dbPeriodicalCheck.schedule(checkForReminders, Calendar.getInstance().getTime(), CheckDatabaseTask.DB_CHECK_INTERVAL);
		dbPeriodicalCheck.schedule(checkForStartingEvents, Calendar.getInstance().getTime(), CheckDatabaseTask.DB_CHECK_INTERVAL);
		dbPeriodicalCheck.schedule(checkForEndingEvents, Calendar.getInstance().getTime(), CheckDatabaseTask.DB_CHECK_INTERVAL);
	}
	...
	public void addBatchReminders(List reminders) {
		...
		for (Iterator it = reminders.iterator(); it.hasNext();) {
			...
			if (o instanceof Integer) {
				try {
					//if already exists do nothing; cause nothing changed for sure. Otherwise updateReminderTask would have been called
					if (!(forthcomingReminders.containsKey((Integer) o))) {				
						Event e = dbServer.getEventFromCalendar(null, ((Integer) o).intValue(), MyAgenda.COMMIT);
						SendContextEventTask tt; 
						Date d = e.getReminder().getReminderTime().toGregorianCalendar().getTime();
						if (e.getReminder().getTimesToBeTriggered() < 0) {
							tt = new SendReminderTask(agendaListener, e, DEFAULT_TRIGGER_TIMES);
						} else {
							tt = new SendReminderTask(agendaListener, e, e.getReminder().getTimesToBeTriggered());
						}
						if (e.getReminder().getRepeatInterval() <= 0) {
							publishContextEvents.schedule(tt, d, DEFAULT_REPEAT_INTERVAL);
						} else {
							publishContextEvents.schedule(tt, d, e.getReminder().getRepeatInterval());
						}
						
						forthcomingReminders.put(new Integer(e.getEventID()), new TimerTaskEntry(tt, d));
					}
			...
			if (o instanceof Integer) {
				try {
					...
					if (!(forthcomingStartEvents.containsKey((Integer) o))) {				
						Event e = dbServer.getEventFromCalendar(null, ((Integer) o).intValue(), MyAgenda.COMMIT);
						SendContextEventTask tt; 
						Date d = e.getEventDetails().getTimeInterval().getStartTime().toGregorianCalendar().getTime();
						tt = new SendStartEventTask(agendaListener, e);
						publishContextEvents.schedule(tt, d);
						forthcomingStartEvents.put(new Integer(e.getEventID()), new TimerTaskEntry(tt, d));
					}
				} catch (NullPointerException npe) {
					Activator.log.log(LogService.LOG_ERROR, "Event Start time is null");
				}
			}
		}
	}
			...
			if (o instanceof Integer) {
				try {
					...
					if (!(forthcomingEndEvents.containsKey((Integer) o))) {				
						Event e = dbServer.getEventFromCalendar(null, ((Integer) o).intValue(), MyAgenda.COMMIT);
						SendContextEventTask tt; 
						Date d = e.getEventDetails().getTimeInterval().getEndTime().toGregorianCalendar().getTime();
						tt = new SendEndEventTask(agendaListener, e);
						publishContextEvents.schedule(tt, d);
						forthcomingEndEvents.put(new Integer(e.getEventID()), new TimerTaskEntry(tt, d));
					}
				} catch (NullPointerException npe) {
					Activator.log.log(LogService.LOG_ERROR, "Event End time is null");
				}
			}
		}
	}
	...
	public void updateReminderTask(Event e) {
		...
		long newReminder = -1;
		try {
			newReminder = e.getReminder().getReminderTime().toGregorianCalendar().getTimeInMillis();
			hasReminderNew = true;
		} catch (NullPointerException npe) {
		...
		//if it does not have a reminder then delete any previous reference and return
		if (!hasReminderNew) {
			return;
		}
		...
		System.err.println("Adding new task for reminder");
		int triggerTimes = e.getReminder().getTimesToBeTriggered();

		if (triggerTimes < 0) 
			triggerTimes = DEFAULT_TRIGGER_TIMES;
		SendReminderTask task = new SendReminderTask(agendaListener, e, triggerTimes);
...
/*
	public void updateReminderTask(Event e) {
		try {
			Date d = e.getReminder().getReminderTime().toGregorianCalendar().getTime();
			long now = Calendar.getInstance().getTimeInMillis();
			int remainingRepeatTimes = MAX_REPEAT_TIMES;
			// has to be added
			if ((now <= d.getTime()) && (d.getTime() < (now + CheckDatabaseTask.DB_CHECK_TIME_PERIOD))) {
				// check if already exists
				if (forthcomingReminders.containsKey(new Integer(e.getEventID()))) {
					TimerTaskEntry tte = removeReminderTask(e.getEventID());
					if (d.equals(tte.executionDate)) {
						remainingRepeatTimes = tte.tt.getRemainingRepeatTimes();
					}
				} else {
					remainingRepeatTimes = MAX_REPEAT_TIMES;
				}
				SendContextEventTask tt = new SendReminderTask(agendaListener, e, remainingRepeatTimes);
				forthcomingReminders.put(new Integer(e.getEventID()), new TimerTaskEntry(tt, d));
				if (e.getReminder().getRepeatTime() != 0) {
					publishContextEvents.schedule(tt, d, e.getReminder().getRepeatTime());
				} else {
					publishContextEvents.schedule(tt, d);
				}
			} else {
				removeReminderTask(e.getEventID());
			}
		} catch (NullPointerException npe) {
			Activator.log.log(LogService.LOG_ERROR, "Event Reminder is null");
		}
	...
	*/
	public void updateReminderTask(int eventID, Reminder reminder) {
		try {
			SendContextEventTask task = (SendContextEventTask) forthcomingReminders.get(new Integer(eventID));
			if (task != null) {
				//cached search
				...
				Event e = dbServer.getEventFromCalendar(null, eventID, MyAgenda.COMMIT);
				if (e != null) {
	...
	public void updateStartEventTask(Event e) {
		...
		long newStartTime = -1;
		try {
			newStartTime = e.getEventDetails().getTimeInterval().getStartTime().toGregorianCalendar().getTimeInMillis();
		...
		//if it does not have a start time then delete any previous reference and return
		if (!hasStartTimeNew) {
			return;
		}
		...
		removeStartEventTask(eventID);
		System.err.println("Adding new task for start");
	...
	public void updateEndEventTask(Event e) {
		...
		long newEndTime = -1;
		try {
			newEndTime = e.getEventDetails().getTimeInterval().getEndTime().toGregorianCalendar().getTimeInMillis();
			hasEndTimeNew = true;
		} catch (NullPointerException npe) {
			hasEndTimeNew = false;
		}
		...
		//if it does not have an end time then delete any previous reference and return
		if (!hasEndTimeNew) {
			return;
		}
	...
	public TimerTaskEntry removeReminderTask(int eventId) {
		...
		TimerTaskEntry tte = (TimerTaskEntry) forthcomingReminders.remove(new Integer(eventId));
		if (tte != null) {
			tte.tt.cancel();
		}
		return tte;
	...
	public TimerTaskEntry removeStartEventTask(int eventId) {
		...
		TimerTaskEntry tte = (TimerTaskEntry) forthcomingStartEvents.remove(new Integer(eventId));
		if (tte != null) {
	...
	public TimerTaskEntry removeEndEventTask(int eventId) {
		...
		TimerTaskEntry tte = (TimerTaskEntry) forthcomingEndEvents.remove(new Integer(eventId));
		if (tte != null) {
	...
	public static void main(String[] str) {
		//XMLGregorianCalendar grCal1 = TypeMapper.getDataTypeFactory().newXMLGregorianCalendar(2009, 2, 11, 16, 00, 00, 0, 2);
		//XMLGregorianCalendar grCal2 = TypeMapper.getDataTypeFactory().newXMLGregorianCalendar(2009, 2, 15, 20, 00, 00, 0, 2);
		//XMLGregorianCalendar grCal3 = TypeMapper.getDataTypeFactory().newXMLGregorianCalendar(2009, 2, 28, 11, 34, 38, 0, 2);
		XMLGregorianCalendar grCal4 = TypeMapper.getDataTypeFactory().newXMLGregorianCalendar(2009, 2, 28, 18, 00, 10, 0, 2);
		XMLGregorianCalendar grCal5 = TypeMapper.getDataTypeFactory().newXMLGregorianCalendar(2009, 3, 12, 18, 00, 00, 0, 2);
		XMLGregorianCalendar grCal6 = TypeMapper.getDataTypeFactory().newXMLGregorianCalendar(2009, 2, 27, 18, 00, 00, 0, 3);
		XMLGregorianCalendar grCal7 = TypeMapper.getDataTypeFactory().newXMLGregorianCalendar(2009, 2, 27, 16, 30, 00, 0, 2);
		XMLGregorianCalendar grCal8 = TypeMapper.getDataTypeFactory().newXMLGregorianCalendar(2009, 3, 29, 23, 23, 45, 0, 3);
		XMLGregorianCalendar grCal9 = TypeMapper.getDataTypeFactory().newXMLGregorianCalendar(2009, 3, 30, 00, 23, 00, 0, 3);

//		System.out.println(grCal1.toGregorianCalendar().getTimeInMillis());
//		System.out.println(new Date(grCal1.toGregorianCalendar().getTimeInMillis()));
//		System.out.println(grCal2.toGregorianCalendar().getTimeInMillis());
//		System.out.println(new Date(grCal2.toGregorianCalendar().getTimeInMillis()));
//		System.out.println(grCal3.toGregorianCalendar().getTimeInMillis());
//		System.out.println(new Date(grCal3.toGregorianCalendar().getTimeInMillis()));
		System.out.println(grCal4.toGregorianCalendar().getTimeInMillis());
		System.out.println(new Date(grCal4.toGregorianCalendar().getTimeInMillis()));
		System.out.println(grCal5.toGregorianCalendar().getTimeInMillis());
		System.out.println(new Date(grCal5.toGregorianCalendar().getTimeInMillis()));
		System.out.println(grCal6.toGregorianCalendar().getTimeInMillis());
		System.out.println(new Date(grCal6.toGregorianCalendar().getTimeInMillis()));
		System.out.println(grCal7.toGregorianCalendar().getTimeInMillis());
		System.out.println(new Date(grCal7.toGregorianCalendar().getTimeInMillis()));
		System.out.println(grCal8.toGregorianCalendar().getTimeInMillis());
		System.out.println(new Date(grCal8.toGregorianCalendar().getTimeInMillis()));
		System.out.println(grCal9.toGregorianCalendar().getTimeInMillis());
		System.out.println(new Date(grCal9.toGregorianCalendar().getTimeInMillis()));
		...
		System.out.println("IS ta: "  + tz.inDaylightTime(new Date(2009, 10, 1)));
