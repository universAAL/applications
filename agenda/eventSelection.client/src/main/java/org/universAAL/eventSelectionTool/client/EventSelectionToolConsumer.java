...
public class EventSelectionToolConsumer extends ContextSubscriber {
	...
	private static final Logger mainLogger = LoggerFactory.getLogger(EventSelectionToolConsumer.class);
	...
	public EventSelectionToolConsumer(BundleContext context) {
		super(context, getContextSubscriptionParams());
		caller = new DefaultServiceCaller(context);	
		
		FilterParams fp = new FilterParams(null);
		fp.setDTbegin(TypeMapper.getDataTypeFactory().newXMLGregorianCalendarDate(2009, 1+1, 27, 2));
		fp.setDTend(TypeMapper.getDataTypeFactory().newXMLGregorianCalendarDate(2009, 1+1, 27+1, 2));
	...
	public List getSelectedEventsService(FilterParams fp, List calendarList){
		...
		if (calendarList == null || calendarList.size() == 0) {
			...
			mainLogger.info(
					"EST\tService called: \'request events\' (" + startTime + ")" + "\n" +
			...
			long endTime = System.currentTimeMillis();
			mainLogger.info(
					"EST\tService called: \'request events from calendars\' (" + startTime + ")" + "\n" +
					"EST\tService returned: \'request events from calendars\' (" + endTime + ")" + "\n" +
					"EST\tTime delay: " + (endTime - startTime));
		...
		if (sr.getCallStatus() == CallStatus.succeeded)  {
			...
			Activator.log.log(LogService.LOG_INFO, "1.Filtered Events have been retreived");
			if ((getEventList == null) || (getEventList.size() == 0)) {
				Activator.log.log(LogService.LOG_INFO, "1.Filtered Events have been retreived: No matching event.");
	...
	public List getSelectedLimitedEventsService(FilterParams fp, List calendarList, int maxEventNo){
		long startTime = System.currentTimeMillis();
		ServiceResponse sr = caller.call(requestFromCalendarLimitedEvents((fp), calendarList, maxEventNo));
		long endTime = System.currentTimeMillis();
		mainLogger.info(
				"EST\tService called: \'get limited events\' (" + startTime + ")" + "\n" +
		...
		if (sr.getCallStatus() == CallStatus.succeeded)  {
			...
			Activator.log.log(LogService.LOG_INFO, "2.Filtered Events have been retreived");
			if ((getEventList == null) || (getEventList.size() == 0)) {
				Activator.log.log(LogService.LOG_INFO, "2.Filtered Events have been retreived: No matching event.");
				return new ArrayList(0);
			}
			return getEventList;
		} 
	...
	public List getFollowingEventsService(List calendarList, int maxEventNo){
		long startTime = System.currentTimeMillis();
		ServiceResponse sr = caller.call(requestFollowingEvents(calendarList, maxEventNo));
		long endTime = System.currentTimeMillis();
		mainLogger.info(
				"EST\tService called: \'get present/future events\' (" + startTime + ")" + "\n" +
				"EST\tService returned: \'get present/future event\' (" + endTime + ")" + "\n" +
				"EST\tTime delay: " + (endTime - startTime));
		...
		if (sr.getCallStatus() == CallStatus.succeeded)  {
			...
			Activator.log.log(LogService.LOG_INFO, "3.Filtered Events have been retreived");
			if ((getEventList == null) || (getEventList.size() == 0)) {
				Activator.log.log(LogService.LOG_INFO, "3.Filtered Events have been retreived: No matching event.");
				return new ArrayList(0);
			}
			return getEventList;
		} 
	...
	private ServiceRequest requestFromCalendarEvents(FilterParams fp, List calList) {
		...
		String[] ppFilterParams = 
				new String[] {EventSelectionToolService.PROP_CONTROLS, EventSelectionTool.PROP_HAS_FILTER_PARAMS};
		if (calList == null) {calList = new ArrayList(0);}
		...
		if (outputs == null) {
			...
			if (output.getURI().equals(expectedOutput))
				if (returnValue == null) {
					...
					if ((returnValue instanceof Resource) && ((Resource)returnValue).getURI().equals(Resource.RDF_EMPTY_LIST))
