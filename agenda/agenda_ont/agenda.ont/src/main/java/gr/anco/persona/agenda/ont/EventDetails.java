...
public class EventDetails extends ManagedIndividual {
	...
	public static Restriction getClassRestrictionsOnProperty(String propURI) {
		...
		if (PROP_HAS_ADDRESS.equals(propURI))
			return Restriction.getAllValuesRestrictionWithCardinality(propURI, PhysicalAddress.MY_URI, 1, 1);
		...
		if (PROP_PARTICIPANTS.equals(propURI))
			return Restriction.getAllValuesRestriction(propURI, TypeMapper.getDatatypeURI(String.class));
		if (PROP_HAS_VALID_PERIOD.equals(propURI))
			return Restriction.getAllValuesRestrictionWithCardinality(propURI, TimeInterval.MY_URI, 1, 0);
	...
	public String toString(String fieldSeperator) {
		...
		s.append("\tSp. Language:\t" + this.getSpokenLanguage() + fieldSeperator);
		if (this.getParticipantList() != null)
			s.append("\tParticipants:\t" + this.getParticipantList().size() + fieldSeperator);
		...
		if (ad == null) {
			...
			if (ad.getCountryName() != null)
				s.append("\t\tCountry  :\t" + ad.getCountryName()[0] + fieldSeperator);
			s.append("\t\tExtended Address :\t" + ad.getExtendedAddress() + fieldSeperator);
			s.append("\t\tLocality     :\t" + ad.getLocality() + fieldSeperator);
			s.append("\t\tPostal Code :\t" + ad.getPostalCode() + fieldSeperator);
			s.append("\t\tRegion   :\t" + ad.getRegion() + fieldSeperator);
			if (ad.getStreetAddress() != null) {
				s.append("\t\tStreet Name  :\t" + ad.getStreetAddress().streetName + fieldSeperator);
				s.append("\t\tBuilding  :\t" + ad.getStreetAddress().buildingIdentifier);
			}
		}
