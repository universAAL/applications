...
import org.persona.platform.profiling.ontology.UserProfile;
...
public class Reminder extends ManagedIndividual {
	...
	public static Restriction getClassRestrictionsOnProperty(String propURI) {
		if (PROP_HAS_TYPE.equals(propURI))
			return Restriction.getAllValuesRestrictionWithCardinality(propURI, ReminderType.MY_URI, 1, 0);		
		...
		if (PROP_TRIGGER_TIMES.equals(propURI))
			return Restriction.getAllValuesRestrictionWithCardinality(propURI, TypeMapper.getDatatypeURI(Integer.class), 1, 0);
	...
	/*
	 * Return the time (in milliseconds) that will be passed before 
	 * the reminder will be re-triggered 
	 */
	...
	public int getPropSerializationType(String propURI) {
		//return PROP_HAS_REMINDER_TYPE.equals(propURI) ? PROP_SERIALIZATION_OPTIONAL : PROP_SERIALIZATION_FULL;
	...
	public String toString(String fieldSeperator) {
		...
		if (this.getReminderTime() != null)
			s.append("\tTime    :\t" + this.getReminderTime().toString() + fieldSeperator);
