...
public class EventComparator implements Comparator {
	...
	public int compare(Object event1, Object event2) {
		try {
			XMLGregorianCalendar x1 = ((Event)event1).getEventDetails().getTimeInterval().getStartTime();
			XMLGregorianCalendar x2 = ((Event)event2).getEventDetails().getTimeInterval().getStartTime();
	...
	public static void main(String[] str){
		XMLGregorianCalendar event1 = TypeMapper.getDataTypeFactory().newXMLGregorianCalendar(2008, 11, 3, 23, 59, 59, 0, +2);
		XMLGregorianCalendar event2 = TypeMapper.getDataTypeFactory().newXMLGregorianCalendar(2008, 10, 1, 23, 59, 59, 0, +2);
		XMLGregorianCalendar event3 = TypeMapper.getDataTypeFactory().newXMLGregorianCalendar(2008, 12, 3, 23, 59, 59, 0, +2);
		...
		for (int i = 0; i < 3; ++i) {
			System.out.println(((Event)list.get(i)).getEventDetails().getTimeInterval().getStartTime());
		}
