...
public abstract class SendVCardConsoleCommand extends ConsoleCommand {
  ...
  public static final int ADD_CONTACT_TYPE = 1;
  ...
  public SendVCardConsoleCommand(String name, String description) {
  ...
  public void executeCommand(String command, int commandType, String... parameters) {
    ...
    boolean res;
    if (commandType == ADD_CONTACT_TYPE) {
      res = sendAddContact(user, personalInformationSubprofile);
    } else if (commandType == EDIT_CONTACT_TYPE){
      res = sendEditContact(user, personalInformationSubprofile);
    } else {
      throw new ContactManagerShellException("Unexpected type of command:" + commandType);
    }
