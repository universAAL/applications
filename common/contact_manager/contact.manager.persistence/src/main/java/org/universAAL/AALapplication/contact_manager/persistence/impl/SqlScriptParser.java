...
public final class SqlScriptParser {
  ...
  private String readNextLine(BufferedReader bufferedReader) throws IOException {
    String line = bufferedReader.readLine();
    if (line == null) {
      throw new ContactManagerPersistenceException("Sql statement is not closed with the delimiter : "
          + statementDelimiter);
    }
    return line.trim();
  }
  ...
  public Map<Integer, String> getSqlStatementMap() {
    if (sqlStatementMap.isEmpty()) {
      throw new ContactManagerPersistenceException("The SQL statement map is empty");
    }
