...
import org.universAAL.LivingRoom.unit_impl.LampStateListener;
import org.universAAL.LivingRoom.unit_impl.MyLighting;
import org.universAAL.LivingRoom.unit_impl.UILampsController;
import org.universAAL.LivingRoom.unit_impl.UILampsView;
...
public class LightingProvider extends ServiceCallee implements
    ...
    private static int hrthreshold = 1;
    private static int hrr = 60;
    /************** THE MAPPING NOTE ****************/
	...
	if (input == null)
	    return null;
	String lampURI = input.toString();
	if (operation.startsWith(ProvidedLightingService.SERVICE_GET_LAMP_INFO))
	    return getLampInfo(lampURI);

	if (operation.startsWith(ProvidedLightingService.SERVICE_TURN_OFF))
	{
		Huemain.huecontrol(46920,255,"2");
		hrthreshold = 1;
		return turnOff(lampURI);
	}
	if (operation.startsWith(ProvidedLightingService.SERVICE_TURN_ON))
	{
		System.out.println("******************turn_ON:");
		Huemain.huecontrol(12750,255,"1");	
		if(hrthreshold!=3)
		hrthreshold=2;
	    return turnOn(lampURI);
	}
	input = call.getInputValue(ProvidedLightingService.INPUT_LAMP_BRIGHTNESS);
	if (input == null && !(input instanceof Integer))
			return invalidInput;

	if (operation.startsWith(ProvidedLightingService.SERVICE_SET_BRIGHTNESS)) {
			if (hrthreshold == 2) {
				hrr = ((Integer) input).intValue();  // set to threshold
				System.out.println("有效心跳:"+hrr);
				hrthreshold = 3;
			}
			if (((Integer) input).intValue() >= hrr && ((Integer) input).intValue() < hrr+20) {  // 有效運動  Green
				Huemain.huecontrol(25500, 255, "3");
				System.out.println("超不爽1");
			} else if(((Integer) input).intValue() >= hrr+20){  // 過度運動  Red
				Huemain.huecontrol(65280, 255, "3");
				System.out.println("超不爽2");
			}
			else if(((Integer) input).intValue()<hrr)//無效運動  Yellow
			{
				Huemain.huecontrol(12751, 255, "3");
				System.out.println("超不爽3");
			}
			System.out.println("超不爽"+"hrr:"+hrr+"((Integer) input).intValue():"+((Integer) input).intValue());
			return setBrightness(lampURI, ((Integer) input).intValue());
		}
		return null;
	}
     ...
     */
    public void lampStateChanged(int lampID, String loc, int isOn) {
 ...
 // New
  	private ServiceResponse setBrightness(String lampURI, int percent) {
  		// Task4.1.2: change the value of the lamp using
  		// "Activator.theServer.setBrightness(extractLocalIDfromLampURI(lampURI), percent);"
  		// and create an appropriate response
  		System.out.println("#setBrightness:"+lampURI+"percent:"+percent);
  		if (percent >= 0 && percent <= 100) {
  		theServer.setBrightness(
  					extractLocalIDfromLampURI(lampURI), percent);
  			return new ServiceResponse(CallStatus.succeeded);
  		} else
  			return invalidInput;
  	}
