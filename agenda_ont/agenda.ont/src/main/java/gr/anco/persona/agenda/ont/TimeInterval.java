...
public class TimeInterval extends ManagedIndividual {
	...
	public static Restriction getClassRestrictionsOnProperty(String propURI) {
		if (PROP_TIME_PERIOD.equals(propURI))
			return Restriction.getAllValuesRestrictionWithCardinality(propURI, TypeMapper.getDatatypeURI(Duration.class), 1, 0);
	...
	public TimeInterval(String uri, XMLGregorianCalendar startTime, Duration duration){
		super(uri);
	...
	public TimeInterval(String uri, XMLGregorianCalendar startTime, XMLGregorianCalendar endTime){
		super(uri);
	...
	/**
	 * better not to be used, in order to avoid inconsistency between duration and start/end times
	 * i.e., cases where endTime - startTime != duration 
	 * @param uri
	 * @param startTime
	 * @param endTime
	 * @param duration
	 *
	public TimeInterval(String uri, XMLGregorianCalendar startTime, XMLGregorianCalendar endTime, Duration duration){
		super(uri);
		if (startTime == null || duration == null)
			throw new IllegalArgumentException();
	...
	*/
	 ...
	 * @param endTime
	 *
	public TimeInterval(String uri, Duration duration, XMLGregorianCalendar endTime){
		super(uri);
	...
	*/
	...
	public void setStartTime(XMLGregorianCalendar startTime) {
		...
		XMLGregorianCalendar temp = startTime;
		temp.add(d);
		setEndTime(temp);
		*/
	}
	...
	private void setTimePeriod(Duration duration) {
		if (duration == null) return;
		props.put(PROP_TIME_PERIOD, duration);
		
		//update end time
		XMLGregorianCalendar temp = (XMLGregorianCalendar) props.get(PROP_START_TIME);
		temp.add(duration);
		setEndTime(temp);
	} */

	public int getPropSerializationType(String propURI) {
		return PROP_END_TIME.equals(propURI) ? PROP_SERIALIZATION_OPTIONAL : PROP_SERIALIZATION_FULL;
		
	}

	public boolean isWellFormed() {
		return (props.containsKey(PROP_START_TIME) && props.contains(PROP_END_TIME));
	}
